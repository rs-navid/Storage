...
const EditModal = (props) => {
  ...
  const handleSaveReceipt = async () => {
    ...
    } else if (props.editingReceipt.address.trim() === "") {
      props.showDialog({ title: "خطا", text: "آدرس معتبر نمی باشد." });
    } else if (props.editingReceipt.phoneNumber.trim() === "") {
      props.showDialog({ title: "خطا", text: "شماره تماس معتبر نمی باشد." });
    ...
    >
      <Form>
        ...
        <div className="field-wrapper field-100">
          <label>آدرس:</label>
          <TextArea placeholder="آدرس" value={props.editingReceipt.address} className="txt-load"  name="address" onChange={handleInput} />
          <label className="lbl-load" onClick={async ()=>{
            const result = await props.getAddress(props.editingReceipt.clientId);
            if (result) {
              props.setEditingReceipt({
                ...props.editingReceipt,
                address: result,
        ...
        <div className="field-wrapper field-100">
          ...
          <TextArea placeholder="شماره تماس" value={props.editingReceipt.phoneNumber} className="txt-load" name="phoneNumber" onChange={handleInput} />
          <label className="lbl-load" onClick={async ()=>{
            const result = await props.getPhoneNumber(props.editingReceipt.clientId);
            if (result) {
              props.setEditingReceipt({
                ...props.editingReceipt,
                phoneNumber: result,
