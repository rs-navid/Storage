...
import { getSubObjects, deleteSubObjects,updateSubObject, createSubObject } from "../../../store/actions/objectActions";
...
const SubsModal = (props) => {
  ...
  const loadSubs = async () => {
    const results = await props.getSubObjects({parentId:props.mainId});
  ...
  const handleEditButtonClick = (id, name) => {
    setEditingObject({id, name, parentId: props.mainId});
    setSubModalStatus(true);
  };

  // Handle on new click
  const handleNewButtonClick = () => {
    setEditingObject({...objectObject, parentId: props.mainId});
    setSubModalStatus(true);
  };
  ...
  const deleteSubs = async () => {
    const resutlt = await props.deleteSubObjects(selectedSubs);
    if (resutlt) {
  ...
  return (
    <Fragment>
      <SubModal
        ...
        createSubObject={props.createSubObject}
        updateSubObject={props.updateSubObject}
        loadData={loadSubs}
      ...
      <Modal
        open={props.open}
        title={`کالا های دسته بندی (${props.mainName})`}
        cancel={() => {
          ...
          {subs.map((item) => {
            return (
              <ListItemWithCheckboxAndEdit
                id={item.id}
                key={item.id}
                onClick={() => handleEditButtonClick(item.id, item.name)}
                onChange={() => handleSelectionChange(item.id)}
              >
                <SubItems data={["نام کالا:", item.name]} />
              </ListItemWithCheckboxAndEdit>
            );
          })}
        </div>
...
export default withRouter(connect(null, { deleteSubObjects, getSubObjects, updateSubObject, createSubObject, showDialog })(SubsModal));
