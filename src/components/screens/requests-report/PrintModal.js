...
import { printRequestsReport } from "../../../store/actions/requestActions";



const PrintModal = (props) => {
  const [date, setDate] = useState(utils("fa").getToday());
  const [title, setTitle] = useState("گزارش درخواست ها");
  ...
  const handlePrint = async () => {
    const  result = await props.printRequestsReport({...props.data, date, title});
...
export default connect(null, { printRequestsReport })(PrintModal);
